...
class CartController extends Controller
{
    ...
    {   
        ...
        $items = ($user != null) ? 
                ...
                ($request->has('temp_user_id') ? Cart::where('temp_user_id', $request->temp_user_id)->get() : [] );
    ...
    {   
        ...
        $owner_ids = ($user_id != null) ?
            Cart::where('user_id', $user_id)->select('owner_id')->groupBy('owner_id')->pluck('owner_id')->toArray() : 
            ($temp_user_id != null ? Cart::where('temp_user_id', $temp_user_id)->select('owner_id')->groupBy('owner_id')->pluck('owner_id')->toArray() : [] );
        ...
        if (!empty($owner_ids)) {
            foreach ($owner_ids as $owner_id) {
                ...
                $shop_items_raw_data = ($user_id != null) ?
                    Cart::where('user_id', $user_id)->where('owner_id', $owner_id)->get()->toArray() : 
                    ($temp_user_id != null ? Cart::where('temp_user_id', $temp_user_id)->where('owner_id', $owner_id)->get()->toArray() : [] );
