...
class ShippingController extends Controller
{
    ...
    public function shipping_cost(Request $request)
    {
        ...
        $main_carts    = ($userId != null) ? Cart::where('user_id', $userId)->get() : Cart::where('temp_user_id', $tempUserId)->get();
        $shipping_info = null;
        foreach ($request->seller_list as $key => $seller) {
            $seller['shipping_cost'] = 0;

            $cartItems = ($userId != null) ? Cart::where('user_id', $userId) : Cart::where('temp_user_id', $tempUserId);
            $carts = $cartItems->where("owner_id", $seller['seller_id'])->get();
        ...
        $total_shipping_cost = $cartItems->sum('shipping_cost');
    ...
    public function getDeliveryInfo(Request $request)
    {
        ...
        $owner_ids = $cartItems->select('owner_id')->groupBy('owner_id')->pluck('owner_id')->toArray();
        $shops = [];
        if (!empty($owner_ids)) {
