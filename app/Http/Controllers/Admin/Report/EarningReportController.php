...
class EarningReportController extends Controller
{
    ...
    public function index()
    {
        ...
        // Payouts data
        $seller_payments = Payment::groupBy('time')
                        ->select(DB::raw('SUM(amount) as total'), DB::raw('DATE_FORMAT(created_at, "%M") AS time'))
                        ->whereYear('created_at', Carbon::now()->year)
                        ->orderBy(DB::raw('MONTH(created_at)'), 'asc')
                        ->get();
        ...
        foreach($data['sales_stat'] as $sale){
            $total_sales_alltime += $sale['total'];

            if($sale['time'] == date('F'))
                $sales_this_month += $sale['total'];
        }
        $data['total_sales_alltime'] = $total_sales_alltime;
    ...
    function sale_analytic(Request $request)
    {
        ...
        $order_query = Order::where('delivery_status', 'delivered')->groupBy('time');
        ...
        $seller_subscriptions = array();
        if (addon_is_activated('seller_subscription')) {
            $seller_subscriptions_query = SellerPackagePayment::groupBy('time');
            if($intervalType == 'MONTH'){
            ...
            $seller_subscriptions = $seller_subscriptions_query->where('approval', 1)->orderBy(DB::raw('Date(created_at)'), 'asc')->get();
        }
        // Earning from Seller Subscription End

        // Earning from Customer Subscription
        $customer_subscription_query = CustomerPackagePayment::groupBy('time');
        if($intervalType == 'MONTH'){
        ...
        $customer_subscriptions = $customer_subscription_query->where('approval', 1)->orderBy(DB::raw('Date(created_at)'), 'asc')->get();
    ...
    function payout_analytic(Request $request)
    {
        $intervalType = $request->interval_type;
        // Seller payments
        $seller_payment_query = Payment::groupBy('time');
        if($intervalType == 'MONTH'){
        ...
        $refunds = array();
        if (addon_is_activated('refund_request')) {
            $refund_query = RefundRequest::groupBy('time')->where('admin_approval', 1);
            if($intervalType == 'MONTH'){
        ...
        $delivery_boy_payments = array();
        if (addon_is_activated('delivery_boy')) {
            $delivery_boy_payment_query = DeliveryBoyPayment::groupBy('time');
            if($intervalType == 'MONTH'){ 
